...
| _this batch_ | **C5** safe IO: the open project is remembered (real quick-save), unsaved changes are guarded before New/Open/Import, edits autosave to a recovery snapshot, and a snapshot left by a previous session is offered for restore | `test/project_session_test.dart` (17 tests); `flutter test`: **102 passed** |

**Phase B is complete.** What is left:

- **A4** — lint ratchet, so CI can fail on warnings and not just errors.
- **Phase C (2 of 6)** — C1 and C5 done. Remaining: C2 preview/commit/cancel so a drag never writes into the document, C3 geometry-based snapping, C4 group enter/select + layers tree + rename, C6 inspector completion (opacity, numeric transform for all types, gradient stop editing).
...
- **Import/open destroys unsaved work silently** — **FIXED** in the C5 batch. A dirty session now prompts Save / Discard / Cancel before New, Open or Import, and a cancelled or failed save cancels the action.
- **No autosave, recovery or file tracking** — **FIXED** in the C5 batch. The open project is remembered, so saving is a real quick-save instead of a Save-As prompt every time (plus an explicit Save As); the title shows a `•` while there are unsaved changes; edits are snapshotted to a recovery file on a debounce; and a leftover snapshot at startup is offered for restore (the file's existence *is* the crash signal, since a clean save deletes it). Closing the window with unsaved changes prompts too, via `AppLifecycleListener.onExitRequested` — fail-open by design, since being unable to close the editor is a worse failure than a missing prompt. *Needs a manual desktop check: window-close behaviour cannot be exercised headlessly.*
